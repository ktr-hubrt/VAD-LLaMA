...
from video_llama.datasets.datasets.video_instruct_dataset import Video_Instruct_Dataset, Video_Instruct_Eval_Dataset
...
class Instruct_Builder(BaseDatasetBuilder):
    ...
    def build(self):
        ...
        if 'eval' in self.vis_processors and 'ucf' in build_info.videos_dir:
            # print(build_info)

            split = 'eval'
            datasets[split] = eval_dataset_cls(
                vis_processor=self.vis_processors[split],
                text_processor=self.text_processors[split],
                vis_root=build_info.videos_dir,
                ann_root=build_info.eval_anno_dir,
                num_video_query_token=num_video_query_token,
                tokenizer_name=tokenizer_name,
                test_flag=self.config.test_flag,
                data_type=self.config.data_type
            )
        return datasets
...
@registry.register_builder("vad_instruct")
class WebvidInstruct_Builder(Instruct_Builder):
    train_dataset_cls = Video_Instruct_Dataset
    eval_dataset_cls = Video_Instruct_Eval_Dataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/instruct/vad_instruct.yaml",
    }
